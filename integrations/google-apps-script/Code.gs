...
function doPost(e) {
  try {
    ...
    try {
      ...
      sheet.appendRow([
        new Date(), clean_(payload.name, 120), clean_(payload.email, 180),
        clean_(payload.phone, 30), clean_(payload.region, 80), clean_(payload.vehicles, 20),
        clean_(payload.type, 80), clean_(payload.problem, 1000), "Sí", "Nuevo", "Landing web",
      ]);
    } finally {
      lock.releaseLock();
    }
...
function validatePayload_(p) {
  if (p.website) return { ok: false, error: "Solicitud inválida." }; // campo trampa antispam
  if (!p.name || !p.email || !p.phone || !p.region || !p.vehicles || !p.type || !p.problem || p.consent !== "on") {
...
function clean_(value, maxLength) {
  ...
  return /^[=+\-@]/.test(text) ? "'" + text : text; // evita fórmulas inyectadas
}
